...
fn test_secret_string_debug_does_not_leak_value() {
    ...
    assert!(
        ...
        "SecretString::Debug must not contain the secret. Got: {debug_output}"
    );
    assert!(
        debug_output.contains("[REDACTED]"),
        "SecretString::Debug must contain [REDACTED]. Got: {debug_output}"
    );
}
...
fn test_secret_bytes_debug_does_not_leak_value() {
    ...
    assert!(
        ...
        "SecretBytes::Debug must not contain raw byte values. Got: {debug_output}"
    );
    assert!(
        debug_output.contains("[REDACTED]"),
        "SecretBytes::Debug must contain [REDACTED]. Got: {debug_output}"
    );
}
