...
mod tests {
    ...
    fn tc_ut_154_build_handle_no_ipc_true_returns_sqlite() {
        ...
        let args = CliArgs::parse_from([
            "shikomi", "--no-ipc", "--vault-dir", dir.path().to_str().unwrap(), "list",
        ]);
        let result = build_handle(&args, Locale::English, false);
        assert!(result.is_ok(), "build_handle --no-ipc should succeed, got: {:?}", result.err());
        assert!(
    ...
    fn tc_ut_155_build_handle_ipc_no_daemon_returns_daemon_not_running() {
        ...
        unsafe { std::env::set_var("XDG_RUNTIME_DIR", xdg_dir.path()); }
        ...
        if let Some(old) = old_xdg {
            ...
            unsafe { std::env::set_var("XDG_RUNTIME_DIR", old); }
        } else {
    ...
    fn tc_ut_159_no_ipc_referenced_in_lib_rs() {
        ...
        let code_refs = non_test_src.lines()
            .filter(|l| {
