...
- PR #29 の runtime reject 経路（`if args.ipc && !matches!(args.subcommand, Subcommand::List)` で `--ipc add/edit/remove` を弾く）の構造的撤去
- `IpcErrorCode → PersistenceError` 写像表に `NotFound` / `InvalidLabel` / `Domain` バリアント追加（list 経路では発生しなかったケース）

## 将来拡張のための設計フック

本 feature の `#[non_exhaustive] enum` 設計と段階移行契約により、以下の後続 feature が**最小コストで積める**。
...
## Phase 1.5（Issue #30）の実装担当への引き継ぎ追補

PR #29（Phase 1）で確立した経路に対し、Phase 1.5 では以下の追加・撤去を行う:
...
- **`IpcVaultRepository::add_record(kind, label, value, now) -> Result<RecordId, PersistenceError>`**: `IpcRequest::AddRecord` を `IpcClient::round_trip` 経由で発行
- **`IpcVaultRepository::edit_record(id, label, value, now) -> Result<RecordId, PersistenceError>`**: 同上、`IpcRequest::EditRecord`
- **`IpcVaultRepository::remove_record(id) -> Result<RecordId, PersistenceError>`**: 同上、`IpcRequest::RemoveRecord`
- **`enum RepositoryHandle { Sqlite(SqliteVaultRepository), Ipc(IpcVaultRepository) }`**: `crates/shikomi-cli/src/lib.rs` 内 non-public enum
- **`run_list` / `run_add` / `run_edit` / `run_remove` の `match handle` 2 アーム分岐**: 各サブコマンドハンドラで Sqlite/Ipc 経路を分岐
- **`From<IpcErrorCode> for PersistenceError` の写像追加**: `NotFound { id }` / `InvalidLabel { reason }` / `Domain { reason }` の 3 バリアント分（PR #29 では list 経路で発生しなかったため未実装）
- **`PersistenceError::RecordNotFound(RecordId)` バリアント新規追加**: IPC 経由 `NotFound` 受信時の写像先（`shikomi-infra::persistence::error::PersistenceError` に追加）

### 設計判断（Issue #30 で確定、案 C 廃止 → 案 D 採用）

- **`IpcVaultRepository` は `VaultRepository` trait を実装しない**: 案 C のシャドウ差分は「嘘の Plaintext(empty) / 嘘 ID / 常時 true な exists()」を構造的に発生させる偽実装だったため**完全廃案**。詳細は `./ipc-vault-repository.md §設計方針の確定` 単一真実源
- **id 生成は daemon 側に集約**: CLI 側の IPC 経路は `Uuid::now_v7()` を**呼ばない**。`IpcResponse::Added { id }` で受け取ったものをそのまま presenter に渡す
- **`Box<dyn VaultRepository>` 抽象化を捨てる**: `enum RepositoryHandle` で網羅性検査を活用

### CI への追加項目（Phase 1.5 実装 PR で対応）

- 本設計 PR では `.github/workflows/` を編集しない（Phase 1 同方針）
- Phase 1.5 実装 PR で以下を追加:
  - `--ipc add` / `--ipc edit` / `--ipc remove` の 3 OS matrix E2E テスト
  - `IpcVaultRepository` の `impl VaultRepository for IpcVaultRepository` ブロックが**存在しない**ことの grep 監査（`crates/shikomi-cli/src/io/ipc_vault_repository.rs` 内に `impl VaultRepository` が 0 件であることを確認）
  - `crates/shikomi-cli/src/io/ipc_vault_repository.rs` 内で `Uuid::now_v7` / `uuid::Uuid::new_v7` 呼出が 0 件であることの grep 監査（id 生成が daemon 側に集約されていることの構造的検証）

## 結語

本 feature の設計は、CLI 直結（Phase 1）と daemon 経由（Phase 1.5）の**経路分離**を `enum RepositoryHandle` + 各サブコマンドハンドラの `match` ディスパッチで明示化する。`cli-vault-commands` で確立した「Composition Root が経路選択の唯一の責務者」が、`IpcVaultRepository` の trait 非実装方針と組み合わさり、**型システムが経路追加時の修正漏れを検出**する保守性を獲得した。

`shikomi-core::ipc` の I/O 不在性、`#[non_exhaustive] enum` による非破壊拡張、`SerializableSecretBytes` の `expose_secret` 不使用契約、`SingleInstanceLock` の race-safe RAII、id 生成の daemon 集約、`IpcVaultRepository` の trait 非実装による偽実装の構造的排除——いずれも**型・責務・依存方向・Fail Fast の契約**として設計書に残した。

実装担当（坂田銀時）は、本設計書の契約を満たす限りにおいて実装詳細を自由に決定できる。後続 feature 担当は `IpcRequest` / `IpcResponse` の `#[non_exhaustive]` 拡張ポイントと `RepositoryHandle` enum の経路追加を組み合わせて、ホットキー / クリップボード / 暗号化 / GUI を**1 バリアント追加 + 1 経路追加 + 1 ハンドラ実装**で積める。
