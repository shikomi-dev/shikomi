...
| **vault.db 不在時の init ログへの secret 混入（REQ-DAEMON-028）** | daemon 初回起動時に `shikomi_daemon::init` target の `tracing::info!` 2 行が出力されるが、その中に secret 値 / vault 内容が混入する経路 | Secret 値 | (1) ログ 1 行目: `"vault not found; created new plaintext vault at {path}"` — `{path}` は **vault_dir（ディレクトリパスのみ、vault ファイル内容でも secret でもない）**。`vault-persistence/basic-design/security.md` の監査ログ規約「vault_dir は INFO 記録可」と整合済み / (2) ログ 2 行目: `"hint: to enable encryption, ..."` — **完全な固定文言**、動的フィールドなし。よって両行とも secret 非含有が構造的に保証される。CI grep 監査（`expose_secret` 0 件）の対象範囲（`crates/shikomi-infra/src/persistence/`）でも同様に検証される |
...
| **IPC エラー `reason` 経由の絶対パス / lock holder PID 漏洩** | 同 UID 悪性プロセスが `IpcRequest::AddRecord` 連発 → daemon が `PersistenceError::Display` を `IpcErrorCode::Persistence.reason` に詰めて返送 → 攻撃者が `"IO error on /home/alice/.local/share/shikomi/vault.db"` / `"vault is locked at ... holder=pid=12345"` から vault 絶対パス / lock holder PID を抜く | ファイルシステム情報 / プロセス情報 | **`IpcErrorCode.reason` をハードコード固定文言のみに限定**（`"persistence error"` / `"domain error"` / `"invalid label"` 等）。`format!("{}", err)` / `err.to_string()` 禁止。詳細観測は daemon 側 `tracing::warn!` のみ（運用者特権、クライアントには開示せず）。`../detailed-design/daemon-runtime.md §Result → IpcErrorCode 写像の規約` が単一真実源（服部平次指摘） |
...
| A09 | Logging Failures | **対応** — `tracing` マクロ呼出時に `IpcRequest` 全体を渡さない。secret 値は `SerializableSecretBytes::Debug` の `[REDACTED]` 固定で保護。panic hook で `payload` 非参照（CI grep 監査）。**[REQ-DAEMON-028 追加]** `shikomi_daemon::init` target の `tracing::info!` 2 行（vault 不在時の init ログ）は vault_dir + static hint のみで secret 非含有——脅威モデル表「vault.db 不在時の init ログへの secret 混入」行で詳細化済み |
...
本 feature は workspace lint `unsafe_code = "deny"` を引き続き尊重するが、以下の領域で **allow オーバーライドを限定許可**する。**配置は服部平次指摘 ② / ③ への対応で 3 領域に固定**:

| crate | モジュール | 許可対象 API | 許可理由 |
|-------|-----------|------------|---------|
| `shikomi-daemon` | `crates/shikomi-daemon/src/permission/unix.rs` | `getsockopt(SO_PEERCRED / LOCAL_PEERCRED)` （`libc` / `nix` 拡張）| ピア UID 取得。`nix` でラップされない低レベル呼出箇所のみ |
| `shikomi-daemon` | `crates/shikomi-daemon/src/permission/windows.rs` | `GetNamedPipeClientProcessId` / `OpenProcessToken` / `GetTokenInformation(TokenUser)` / `ConvertSidToStringSidW` / `GetCurrentProcessToken` | ピア SID 検証（接続ごと）+ **`resolve_self_user_sid` 経由の自 SID 取得**（`lifecycle::socket_path::resolve_pipe_name` から呼ばれる、服部平次指摘 ②）|
| `shikomi-cli` | `crates/shikomi-cli/src/io/windows_sid.rs` | `GetCurrentProcessToken` / `GetTokenInformation(TokenUser)` / `ConvertSidToStringSidW` | CLI 側の自 SID 取得（`default_socket_path` から呼ばれる、daemon 側と同等機能を crate 境界尊重で独立実装、服部平次指摘 ③）|

**`unsafe` を書かない領域（規約）**:

- daemon 側:
  - `crates/shikomi-daemon/src/lifecycle/**`（`socket_path.rs` 含む）— SID 取得は `permission::windows::resolve_self_user_sid()` 経由で呼ぶのみ
  - `crates/shikomi-daemon/src/ipc/**`
  - `crates/shikomi-daemon/src/lib.rs` / `main.rs` / `panic_hook.rs` / `error.rs`
- CLI 側:
  - `crates/shikomi-cli/src/io/ipc_vault_repository.rs` / `ipc_client.rs` — SID 取得は `crate::io::windows_sid::resolve_self_user_sid()` 経由で呼ぶのみ
  - `crates/shikomi-cli/src/**`（`io/windows_sid.rs` 以外の全ファイル）
- `shikomi-core::ipc` 全体（純粋ドメイン）

**3 領域の冒頭に `#![allow(unsafe_code)]` を明示**（ファイル単位の lint オーバーライド）。

**監査**:
- `crates/shikomi-daemon/src/` 配下で `unsafe` ブロックが `permission/{unix,windows}.rs` 以外に出現しないこと（CI grep）
- `crates/shikomi-cli/src/` 配下で `unsafe` ブロックが `io/windows_sid.rs` 以外に出現しないこと（CI grep、本 feature で監査対象に**追加**）
- `crates/shikomi-core/src/` 配下（`ipc/` 含む）で `unsafe` ブロックが出現しないこと（既存監査範囲を維持）
- テスト設計担当への引き継ぎ: 上記 3 監査を **TC-CI-023 / TC-CI-024 / TC-CI-025** 相当で一意採番。`test-design/ci.md` に追加
