...
| TC-UT-142 | 異常（書き込み不可、`#[cfg(unix)]`） | `tempfile::TempDir` のパーミッションを `0o500`（読み取り専用ディレクトリ）に変更後に `load_or_create_plaintext` を呼ぶ | `Err(PersistenceError::InvalidPermission)` が返る。**根拠**: `load_or_create_plaintext` → `load_inner` → `verify_dir` が vault.db 存在確認（Step 5）より先に `0o500 != 0o700` を検出し `InvalidPermission` を返す。vault.db 書き込みステップ（Step 6）には到達しないため `Io(_)` ではなく `InvalidPermission` が正しい期待値（涅マユリ TC-UT-142 実走確認 / PR #124 rev.2 バグレポートによる修正）|
| TC-UT-142w | **Windows 対応** — 除外根拠 | Windows では `fs::set_permissions` でディレクトリの書き込みを禁止するには管理者権限が必要で、CI 環境（非 admin runner）での再現が構造的に困難 | 除外根拠: CI 非 admin 制約。実装担当は `#[cfg(windows)]` で `// NOTE: write-protected directory test skipped on Windows (requires admin privileges)` を残すこと |
