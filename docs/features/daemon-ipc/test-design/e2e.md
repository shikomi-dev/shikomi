...
| ツール選択 | **`expectrl` crate（推奨、第一候補）** または `rexpect`（第二候補）。両者ともクロスプラットフォーム pty を提供。**`expectrl` 採用根拠**: (a) async 対応 + より新しめのメンテ（最終リリース 2024 以降）、(b) `Session::expect` API が文字列マッチに必要十分、(c) **`MIT` ライセンス単独**（`cargo deny check licenses` の `[licenses].allow` に `MIT` が含まれるため整合。**訂正履歴**: 工程2 の `b50e15d` 時点では `MIT OR Apache-2.0` と誤記、服部 review `2798473` 後で crates.io 実証拠と照合し `MIT` 単独へ訂正、Issue #33）、(d) 依存 crate 数 < 10 で重量級でない、(e) **`nix 0.26.4` を `ptyprocess` 経由で間接導入**するが、本 crate は既に `nix 0.29` を直接依存しているため `cargo deny` の `bans.multiple-versions = "deny"` に抵触する。**`deny.toml` の `bans.skip` に `{ name = "nix", version = "0.26.4" }` を追加**して dev-dep 限定の重複を許容（`2798473` で対応済）。`rexpect` は同等機能だが Linux/macOS 限定で Windows サポートが弱い。**dev-deps に追加要請**（Cargo.toml の `[target.'cfg(unix)'.dev-dependencies]` に閉じ込め、本番バイナリ非混入）、`#![cfg(unix)]` ガード（Windows は `ConsoleScreenBuffer` 経路で別観測手段が必要、本 TC は Unix 限定 + `#[ignore]` で Windows skip） |
