...
| `Rename` | `std::fs::rename`（OS 抽象、Unix `rename(2)` / Win `MoveFileExW`）。`ReplaceFileW` 直接採用しない（unsafe FFI 境界追加回避、KISS）。Win 限定で一過性エラー（`ERROR_ACCESS_DENIED` / `ERROR_SHARING_VIOLATION` / `ERROR_LOCK_VIOLATION`）に対する **50ms ± jitter × 最大 5 回 retry**（合計 ≤ 約 250ms）+ retry 直前 symlink 再検証 + 監査ログ記録を本 stage に内包。retry 全敗で本 stage 失敗として上位へ return（Issue #65、`../basic-design/error.md` §Windows rename PermissionDenied 行 / `../basic-design/security.md` §atomic write の二次防衛線）|
