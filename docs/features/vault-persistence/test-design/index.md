...
| `integration.md` | 結合テスト設計（TC-I01〜TC-I20）詳細 |
| `unit.md` | ユニットテスト設計（TC-U01〜TC-U12）詳細 |

---

## 2. テスト対象と受入基準

> **注**: AC-01〜AC-13 は `docs/features/vault-persistence/requirements-analysis.md` §受入基準に定義。AC-14 は本レビューラウンドでの差し戻し指摘を受け、requirements-analysis.md §受入基準に追加された項目（save 側 `.new` 残存検出）。
...
|---------|-----------|-------|---------|------------|------|
| TC-I01 | AC-01 | REQ-P01〜P12 | `cargo doc -p shikomi-infra --no-deps` が成功し公開型が出力される | 結合 | 正常系 |
...
| TC-U01 | — | REQ-P08 | `VaultPaths::new(dir)` のパス導出が正しい | ユニット | 正常系 |
...
テスト戦略ガイドの方針「エンドユーザー操作（UI/CLI/公開API）がない場合は結合テストで代替可」に従い、E2E は本 feature では設計対象外とする。エンドユーザーが `shikomi-infra` を直接触れるのは後続 Issue（`shikomi-cli` / `shikomi-daemon`）が公開される段階。受入基準 AC-01〜AC-14 は `integration.md` の結合テストで網羅する。
...
| 観点 | 基準 |
|------|------|
| 受入基準の網羅 | AC-01〜AC-14 全項目が TC-I01〜TC-I20 のいずれかで網羅されていること（§3 テストマトリクス参照） |
...
| Fail Secure ケース | `.new` 残存 load 側（TC-I05）・`.new` 残存 save 側（TC-I15）・OS パーミッション異常（TC-I07, TC-I12, TC-U10, TC-U12）・暗号化モード拒否（TC-I03, TC-I04）が**必須**（省略不可） |

---

*作成: 涅マユリ（テスト担当）/ 2026-04-23*
*改訂: 涅マユリ（テスト担当）/ 2026-04-23 — レビュー差し戻しに伴う修正: ① `test-design/` ディレクトリ分割 ② AC-14 追加（TC-I15 トレーサビリティ修正） ③ TC-I06 を決定的テストに変更（`write_new_only` フック） ④ §6 実行環境に `serial_test` 明記*
