...
## TC-U26: PermissionGuard::verify_file — ACE に継承フラグ（AceFlags != 0）→ AceFlags==0 違反（Windows）
...
| 種別 | 異常系 |
| 前提条件 | `#[cfg(windows)]`。`ensure_file` 適用済みファイルに対し、`InitializeAcl` + `AddAccessAllowedAceEx(AceFlags=CONTAINER_INHERIT_ACE=0x02)` で ACE=1・`EXPECTED_FILE_MASK`・owner SID の DACL を構築し、`SetNamedSecurityInfoW(DACL_SECURITY_INFORMATION \| PROTECTED_DACL_SECURITY_INFORMATION, ...)` で適用する。`SE_DACL_PROTECTED`・AceCount=1・AceType=ACCESS_ALLOWED・owner SID・`EXPECTED_FILE_MASK` は全て維持し、AceFlags だけを `0x02` にする（不変条件①②③④ の他の条件はすべて通過させ、AceFlags チェックのみを違反させる） |
| 操作 | `PermissionGuard::verify_file(file_path)` を呼ぶ |
