...
| エラー時 | — |
| **Sub-F (#44) 完了後の状態（Boy Scout）** | **本 REQ-CLI-009 は役目終了**。Sub-F が `shikomi vault {encrypt, decrypt, unlock, lock, change-password, recovery-show, rekey, rotate-recovery}` の 8 サブコマンドを提供した時点で「暗号化モード即時拒否」経路は不要となり、`vault-encryption` feature の **REQ-S16「保護モード可視化」** に責務統合される（既存 `add` / `list` / `edit` / `remove` のロック時挙動は MSG-S09(c) + 終了コード 3 の `vault unlock` 誘導に置き換わる）。本 REQ は**履歴として残す**が、実装側は Sub-F 工程3 で `cli/src/lib.rs::run` の `Vault::protection_mode() == Encrypted` 判定経路を **`IpcResponse::Records.protection_mode` 経由に置換**する（Phase 2 規定「CLI は vault に直接触らない」を完成）|
...
| MSG-CLI-103（**Sub-F (#44) 完了後は履歴**）| `error: this vault is encrypted; encryption is not yet supported in this CLI version` | `error: この vault は暗号化されています。本バージョンの CLI は暗号化モード未対応です` | `hint: future "shikomi vault decrypt" will convert it; for now, use a plaintext vault` / `hint: 将来の "shikomi vault decrypt" で変換可能になります。暫定的には平文 vault をご利用ください` | `Vault::protection_mode() == Encrypted`（Sub-F 完成後は `vault-encryption` feature の MSG-S09(c)「Locked → unlock 案内」に責務統合、本 MSG ID は Sub-F 工程3 実装で削除）| 3 |
...
| **`vault-encryption`（Issue #37 Epic、Sub-F #44 で CLI 完成）** | `shikomi vault encrypt` / `decrypt` / `unlock` / `lock` / `change-password` / `recovery-show` / `rekey` / `rotate-recovery` の 8 サブコマンドを Sub-F が実装。本 feature の MSG-CLI-103 は Sub-F 完成で役目終了 → MSG-S09(c) + REQ-S16 に統合（Boy Scout） | `docs/features/vault-encryption/` |
