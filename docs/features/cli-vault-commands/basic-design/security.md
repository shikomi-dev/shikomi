...
**契約**: CLI 層（`shikomi-cli`）内で `SecretString::expose_secret()` / `SecretBytes::expose_secret()` を呼び出すのは以下の**1 箇所のみ**:

- `shikomi-cli/src/usecase/add.rs` 内の `Record::new(..., RecordPayload::Plaintext(input.value), ...)` 構築式
  - `RecordPayload::Plaintext(SecretString)` は `SecretString` 値を**所有権移動**で受け取る設計（expose なし、値のまま格納）
  - つまり**厳密には `expose_secret()` 呼び出しは `shikomi-cli` 内に 0 箇所**を目指す。`SecretString` は生成〜格納まで一度も中身を見ずに運搬する
- `shikomi-cli/src/usecase/edit.rs` 内の `with_updated_payload(RecordPayload::Plaintext(input.value.unwrap()), now)` も同様（expose なし）
...
**テスト担当への引き継ぎ**: `test-design.md` の `§10. CI 補助テスト（TC-CI-xxx）` に以下を追加:

- **TC-CI-013（新設）**: `crates/shikomi-cli/src/**/*.rs` に対し `grep -n "expose_secret"` の結果が 0 件であることを検証
- **TC-CI-014（新設）**: `crates/shikomi-cli/src/**/*.rs` に対し `grep -nE "tracing::(trace|debug|info|warn|error)!.*(panic|PanicHookInfo|info\.payload|info\.message)"` の結果が 0 件であることを検証
- **TC-CI-015（新設）**: `crates/shikomi-cli/src/lib.rs` で `SqliteVaultRepository` への参照が `run()` 関数内の 1 箇所のみであることを grep で検証（`SqliteVaultRepository` 具体型が UseCase / Presenter に漏れ出していないことの証明）
