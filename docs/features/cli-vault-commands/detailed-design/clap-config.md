...
- フィールド:
  ...
  - `ipc: bool` — `#[arg(long = "ipc", global = true)]` （**Issue #26 / `daemon-ipc` feature で追加、Issue #30 で全サブコマンド対応**）
    - daemon 経由経路に切替えるオプトインフラグ
    - 既定（`false`） → 既存の `SqliteVaultRepository::from_directory(...)` 直結経路（`RepositoryHandle::Sqlite`）
    - `true` → `IpcVaultRepository::connect(&default_socket_path()?)` 経由で daemon に接続（`RepositoryHandle::Ipc`）
    - **Issue #30 までの対応サブコマンド**: `list` / `add` / `edit` / `remove` の **4 サブコマンド全て**で `--ipc` を受理（Issue #26 / PR #29 では `list` のみ受理し `add` / `edit` / `remove` は runtime reject していた、Issue #30 でその reject を撤去）
    - daemon 未起動時は `CliError::DaemonNotRunning` で Fail Fast（終了コード 1、`MSG-CLI-110`）
    - **runtime reject 経路の撤去**: PR #29 で `lib.rs::run()` 内に存在した「`args.ipc && !matches!(args.subcommand, Subcommand::List)` → `CliError::UsageError("--ipc currently supports only the `list` subcommand; ...")`」分岐を **Issue #30 で削除する**。サブコマンド網羅は `RepositoryHandle::Ipc` 経路の `match` ディスパッチで型レベルに保証される
    - 詳細: `./composition-root.md §RepositoryHandle 経路ディスパッチ` / `docs/features/daemon-ipc/detailed-design/ipc-vault-repository.md §設計方針の確定`
...
- `--ipc list` 指定 → `args.ipc == true` で `IpcVaultRepository::list_summaries` 経路（PR #29 既存）
- `--ipc add --kind text --label L --value V` → `IpcVaultRepository::add_record` 経路（**Issue #30 新規**、daemon 経由で id 受領 → stdout `added: <id>`）
- `--ipc edit --id <id> --label L2` → `IpcVaultRepository::edit_record` 経路（**Issue #30 新規**）
- `--ipc remove --id <id> --yes` → `IpcVaultRepository::remove_record` 経路（**Issue #30 新規**）
- `--ipc edit --id <非存在 id> --label L2` → daemon `IpcResponse::Error(NotFound)` → `CliError::RecordNotFound` → `MSG-CLI-106` + 終了コード 1（**Issue #30 新規**）
- 旧 PR #29 の runtime reject 経路（`shikomi --ipc add` で「`--ipc currently supports only the list subcommand`」エラー）が Issue #30 で**完全撤去されたこと**の検証（その挙動の E2E テストを Issue #30 で削除）
