...
#### 確定 E: `convco` の commit-msg 呼び出し CLI — **`convco check-message <file>`**

convco 公式 README（https://github.com/convco/convco）の "commit-msg" 統合例セクションに準拠する。`convco check-message` は単一ファイル引数を取り、Conventional Commits 1.0 仕様に合致しない場合に非 0 で exit。`convco check [REVRANGE]` は履歴範囲検証専用であり commit-msg フックには不適。

レシピ定義: `just commit-msg-check {{FILE}}` → 本体は `convco check-message {{FILE}}` のみ。

出典: https://github.com/convco/convco README §Git hooks → commit-msg セクション
...
| 定数名 | 例 | 用途 |
|-------|-----|------|
| `LEFTHOOK_VERSION` | `1.7.18` | ピンバージョン。実値は Sub-issue C の初期実装で確定 |
...
| `test-cli` | なし | `cargo test -p shikomi-cli` | （新設。CI 側の対応ワークフローは Sub-issue A で判断し既存と整合） |
...
| `commit-msg-check` | `file` 引数 1 個 | `convco check-message {{file}}`（確定 E: convco 公式 README の commit-msg 統合例に準拠） | — |
...
**ピン同期の担保**: `setup.sh` と `setup.ps1` の `LEFTHOOK_VERSION` / SHA256 値が乖離すると、Windows 開発者だけ別バイナリを引く事故が起きる。CI に「両ファイルから同値を grep で抽出し diff する簡易検査」を追加する検討は Sub-issue C 実装時に行う（実被害が出る前に自動化しておく価値があるか判定）。
...
| 6 | **gitleaks setup（GitHub Releases + SHA256 検証）** | setup スクリプトと同ロジックをワークフロー内に inline で書くか、setup.sh を流用するかは Sub-issue B で決定 |
