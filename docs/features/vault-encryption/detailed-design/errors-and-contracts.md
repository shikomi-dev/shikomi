...
| **C-20** `vault decrypt` は `DecryptConfirmation` 引数必須、`--force` でも省略不可（Sub-D 新規、Rev2 で Clean Arch 整合） | `VaultMigration::decrypt_vault(.., confirmation: DecryptConfirmation)` 型シグネチャ強制 + `_private: ()` 非可視性フィールドで外部 crate 直接構築禁止（`E0451`）。`DecryptConfirmation::confirm() -> Self` のみが pub コンストラクタ（引数ゼロ）。確認ロジック（キーワード入力 + パスワード再入力 + ConstantTimeEq 比較）は Sub-F CLI/GUI 層責務（`repository-and-migration.md` §`DecryptConfirmation` Clean Architecture 観点の責務分離 表）。 | compile_fail doc test: `decrypt_vault(password)` （confirmation なし）がコンパイルエラー + 外部 crate からの `DecryptConfirmation { _private: () }` 直接構築が `E0451` |
