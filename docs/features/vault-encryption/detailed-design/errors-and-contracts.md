...
| **C-20** `vault decrypt` は `DecryptConfirmation` 引数必須、`--force` でも省略不可（Sub-D 新規、Rev2 で Clean Arch 整合） | `VaultMigration::decrypt_vault(.., confirmation: DecryptConfirmation)` 型シグネチャ強制 + `_private: ()` 非可視性フィールドで外部 crate 直接構築禁止（`E0451`）。`DecryptConfirmation::confirm()` のみが pub コンストラクタ。**Sub-D Rev2**: 旧 `confirm(yes_keyword, password_reentry, expected_password) -> Result<_, ConfirmError>` シグネチャは Clean Arch 違反（`subtle::ConstantTimeEq` を shikomi-infra に持ち込む）のため引数ゼロ `confirm() -> Self` に簡略化、確認ロジック（`"DECRYPT"` キーワード入力 + パスワード再入力 + ConstantTimeEq 比較）は **Sub-F CLI/GUI 層**に責務移譲（`repository-and-migration.md` §`DecryptConfirmation` Clean Architecture 観点の責務分離 表）| compile_fail doc test: `decrypt_vault(password)` （confirmation なし）がコンパイルエラー + 外部 crate からの `DecryptConfirmation { _private: () }` 直接構築が `E0451` |
