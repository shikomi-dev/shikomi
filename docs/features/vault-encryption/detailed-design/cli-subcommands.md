...
- `printpdf` 等の追加 crate は MSG-S18 経路で必要。Sub-F PR レビューで `tech-stack.md` §4.6 に minor / major pin で追加。**`liblouis` FFI bindings は不採用**（unsafe C-FFI 経路を増やさない設計判断、§セキュリティ設計 §新規依存の監査 §`--output braille` 行参照）、点字 BRF 出力は **自前 wordlist 変換テーブル方式**を採用（BIP-39 24 語の Grade 2 英語点字は 24 行のシンプル変換テーブルで完全表現可能、軽量で監査容易）。候補比較は §設計判断 + §セキュリティ設計 §新規依存の監査 を参照
...
**採用根拠**: A 案が Sub-A「滞留時間最小化」+ アクセシビリティ + シンプルさを同時達成。`vault encrypt --output braille > recovery.brf` のようなリダイレクトに対しては **shikomi-cli 内部で `umask(0o077)` を出力直前に適用**してから stdout 書出を行うため、ユーザのリダイレクト先ファイルが所有者限定 `0600` 相当で生成される（§セキュリティ設計 §`--output print` / `--output braille` の一時ファイル / リダイレクト対策）。保管後の `shred` / `secure delete` 案内は補助的にユーザに提示するが、**ファイルパーミッションの安全側の既定値は shikomi が責任を持つ**。
...
| `--output print` | stdout（PDF バイナリ）| ハイコントラスト PDF（黒地白文字、最大 36pt、各語に番号付与）。**shikomi-cli 内部で `umask(0o077)` を出力前に適用**してファイル生成、ユーザのリダイレクト先（`> recovery.pdf`）が所有者限定 `0600` 相当で生成される（§セキュリティ設計 §`--output print` / `--output braille` の一時ファイル / リダイレクト対策）| `printpdf` v0.7+（major pin、§セキュリティ設計参照） |
| `--output braille` | stdout（`.brf` テキスト）| Braille Ready Format、Grade 2 英語点字標準。同上の `umask(0o077)` 内部適用 | **自前 wordlist 変換テーブル**（追加 crate なし、`liblouis` FFI bindings は**不採用** = unsafe C-FFI 経路を増やさない設計判断、§セキュリティ設計 §新規依存の監査 §`--output braille` 行参照） |
...
**自動切替経路**: `SHIKOMI_ACCESSIBILITY=1` 環境変数 / OS スクリーンリーダー検出 / 明示フラグのいずれかで `--output screen` 既定を `Braille` に上書き（明示 `--output` フラグは常に最優先）。OS スクリーンリーダー検出の現行実装と Phase 8 引継ぎ:

- **macOS**: 工程5 ペテルギウス指摘により本実装は **Phase 8 に明示的に先送り**。当面は `VOICEOVER_RUNNING=1` 環境変数 hint 経路のみで判定する (subprocess 起動コスト + `defaults read com.apple.universalaccess` のキー名 macOS バージョン依存性 + sandbox/権限分離の再評価が必要なため)。Phase 8 で `Cocoa` accessibility API 直接呼出 (`NSWorkspace`) または `defaults` subprocess 経由で本実装する。`screen_reader.rs:is_screen_reader_active_impl` の macOS arm はこの仕様に整合。
- **Linux**: `pgrep -x orca` プロセス検出で確定実装 (Orca DBus 経路の重い `zbus` 依存を回避、検出精度は同等)。
- **Windows**: PowerShell `Get-Process Narrator` の subprocess 起動 + exit code 判定で確定実装 (PowerShell は Windows 標準同梱、追加依存なし)。
...
| 攻撃経路 | 対処策 |
|---|---|
| `> recovery.pdf` リダイレクトでディスク永続化 | **shikomi-cli 実装が `--output {print,braille}` 経路で `umask(0o077)` を出力直前に内部適用**してから stdout 書出を行う（既存プロセス umask を `mem::replace` 相当で保存 → `0o077` に設定 → 出力 → 元値に復元、Rust では `nix::sys::stat::umask` / Windows は ACL 経由）。これにより `> recovery.pdf` のリダイレクト先ファイルは**所有者限定 `0600` 相当**で生成され、後続の他プロセス読取を OS 層で構造的に拒否する。**TC-F-A05 で `umask(0o077)` 内部適用 + 出力ファイル mode 0600 を機械検証**。保管後の `shred` / `secure delete` 案内は MSG-S18 で補助的に提示するが、**ファイルパーミッションの安全側既定は shikomi 側責務**（ユーザ責務に丸投げしない、服部 Rev1 再指摘解消）|
...
### 新規依存の監査（printpdf / sys-locale / toml、liblouis FFI 不採用）
...
| 契約 | 強制方法 | 検証手段 |
|---|---|---|
| **C-33**: i18n 辞書キー欠落時もパニックさせず英語 fallback で fail-soft（Sub-F 新規）| `shikomi_cli::i18n::Localizer::translate(key)` が `String` を返し、欠落時は `format!("[missing:{key}]")` を返す | ユニットテスト（TC-F-U02、Sub-F test-design SSoT 整合）|
...
| **C-39**: 24 語出力先 `--output` フラグは排他指定、`screen` 既定 + アクセシビリティ自動切替（Sub-F 新規）| `clap::ValueEnum` 派生で 4 値排他、`accessibility::output_target::resolve` で env / OS 判定 → 自動切替 | unit test（TC-F-U14、Issue #76 工程2 で旧 TC-F-U08 からリナンバ — Issue #75 で TC-F-U08 が `windows_pipe_name_from_dir` 純関数性に占有されたため）|
...
3. **`tech-stack.md` §4.6 に `printpdf` / `sys-locale` 追加 + liblouis FFI 不採用方針記載**（自前 wordlist 変換テーブル、Sub-F #44 Rev1 で凍結済）
...
| Boy Scout | TC-F-U08（`crates/shikomi-cli/src/io/ipc_vault_repository.rs::windows_pipe_name_tests`、Issue #75 で実装）で `windows_pipe_name_from_dir` の**純関数性**（同一 `<DIR>` での決定性 / 16 文字固定長 / Base32 lowercase no-pad alphabet `a-z2-7` / 異なる `<DIR>` 間の衝突耐性）を 4 ケースで機械検証し、CLI / daemon 間の pipe 名導出契約（DRY）が崩れないことを固定。`--vault-dir` の clap parse + 解決順序の検証は TC-F-I12（`crates/shikomi-cli/tests/vault_subcommands.rs`、#74-C で実装）で daemon 不在時の MSG-S09(b) 文言と exit code 64 と併せて E2E 経路で機械検証する。TC-F-U09（同モジュール）は `connect_with_vault_dir` が `--vault-dir` 指定 + 全経路 daemon 不在時に `DaemonNotRunning(primary)` を返すこと（Bug-F-009 Option α）を機械検証 |
