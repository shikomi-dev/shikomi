...
## 分冊構成（Sub-F 完了時点、**Sub-E の 8 分冊 + cli-subcommands.md 新設で 9 分冊**）
...
| **[`vek-cache-and-ipc.md`](./vek-cache-and-ipc.md)（Sub-E 新規）** | VEK キャッシュ + IPC V2 拡張（shikomi-daemon、横断的変更で daemon-ipc feature と双方向同期） | `VekCache` + `VaultUnlockState` enum（`Locked` / `Unlocked { vek, last_used }`、`#[non_exhaustive]`、契約 C-22〜C-25）/ `IdleTimer`（60 秒ポーリング、アイドル 15 分タイムアウト）/ `OsLockSignal` trait（`#[cfg]` 分割、macOS / Windows / Linux 具象実装 + Mock）/ `UnlockBackoff`（連続失敗 5 回で指数バックオフ、契約 C-26）/ IPC V2 5 新 variant ハンドラ（unlock / lock / change_password / rotate_recovery / rekey）/ `MigrationError → IpcError` マッピング（9 → 4 集約、`RecoveryRequired → MSG-S09(a)` 統合、契約 C-27）/ V1 クライアント拒否（`ProtocolDowngrade`、契約 C-28）/ cache_relocked: bool 契約（C-30〜C-32）|
| **[`cli-subcommands.md`](./cli-subcommands.md)（Sub-F 新規）** | shikomi-cli vault サブコマンド + 既存 CRUD のロック時挙動 + 保護モードバナー（横断的変更で daemon-ipc feature `IpcResponse::Records` 構造体化） | `Subcommand::Vault(VaultSubcommand)` の 8 サブコマンド（encrypt / decrypt / unlock / lock / change-password / recovery-show / rekey / rotate-recovery）/ `usecase::vault::*` 8 usecase / `presenter::recovery_disclosure`（24 語 1 度表示 + Drop zeroize 連鎖）/ `presenter::mode_banner`（`[plaintext]` / `[encrypted, locked]` / `[encrypted, unlocked]`、ANSI カラー + 文字二重符号化）/ `presenter::cache_relocked_warning`（MSG-S20 連結 + 再 unlock 案内）/ アクセシビリティ 3 経路（`--print` PDF / `--braille` BRF / `--audio` OS TTS）/ i18n 翻訳辞書 `messages.toml`（ja-JP / en-US）/ 契約 C-33〜C-37（i18n fallback / paste 抑制 / recovery 二度目拒否 / cache_relocked 終了コード 0 / 隠蔽オプション禁止）|
...
- **Sub-F（完了、本書 Rev により本項目は履歴）**: `shikomi vault {encrypt, decrypt, unlock, lock, change-password, recovery-show, rekey, rotate-recovery}` の 8 サブコマンド clap 派生型 + 8 usecase 層、`presenter::recovery_disclosure` / `mode_banner` / `cache_relocked_warning` 新設、アクセシビリティ 3 経路（`--print` / `--braille` / `--audio`）、i18n 翻訳辞書 `messages.toml`（ja-JP / en-US）、既存 CRUD のロック時挙動（MSG-S09(c) + 終了コード 3）、保護モードバナー（REQ-S16）、契約 C-33〜C-37 追加、MSG-S07 / S11 / S14 / S18 / S20 CLI 最終文言確定 → **新規 `cli-subcommands.md`** + `requirements.md` REQ-S15/S16 確定 + 画面・CLI 仕様書き起こし + `basic-design/processing-flows.md` F-F1〜F-F9 処理フロー追記 + `basic-design/ux-and-msg.md` Sub-F 文言索引追加。**横断的変更**: `daemon-ipc` feature の `IpcResponse::Records` 構造体化（`protection_mode` 同梱、`#[non_exhaustive]` 後方互換）、既存 `cli-vault-commands` feature の MSG-CLI-103 役目終了反映（Boy Scout）
