...
| 項目 | 内容 |
|-----|------|
| **能力** | 現在の OS ユーザ権限で動作する別プロセス（マルウェア / 誤実行スクリプト / 同居アプリの脆弱性経由）。`~/.config/shikomi/vault.db` を OS パーミッション内で **read / write 可能**。**daemon UDS / Named Pipe IPC エンドポイントへも接続可能**であり、daemon が未起動の隙に**同名 UDS パスを race pre-create して fake daemon を成立させ、CLI / GUI からのマスターパスワード平文を横取りする経路も成立しうる**。**daemon プロセスのメモリ空間には到達不可**（OS のプロセス分離による） |
| **想定具体例** | ブラウザ拡張のマルウェア化、ダウンロードした不審スクリプト、サプライチェーン攻撃された開発ツール、メーラ添付の自動実行、daemon 未起動時の race pre-create による fake daemon、**CLI 攻撃面**（shell history `~/.bash_history` / `~/.zsh_history`、TTY scrollback、tmux/screen logging、`script` コマンド、core dump、`/tmp` 一時ファイル、`> file.pdf` リダイレクト、`--output audio` TTS dictation 学習 DB、env 仕込みでの fault-injection seam 発火）— **詳細な攻撃経路と対処策は Sub-F `detailed-design/cli-subcommands.md` §セキュリティ設計（CLI 攻撃面）を SSoT 参照**（Sub-F Rev1 で服部指摘により追補）|
| **対応する STRIDE** | **Spoofing**（同名 UDS の race pre-create / fake daemon による IPC 偽装、マスターパスワード横取り経路）、Tampering（vault.db 書換）、Information Disclosure（vault.db 直接読取 + 偽装 daemon へのパスワード流入）、DoS（vault.db 削除 / 破損注入 / IPC エンドポイント占拠による daemon 起動阻害） |
| **対策（暗号化モード）** | (a) **AES-256-GCM の認証タグ**で改竄検出 → `CryptoError::AeadTagMismatch` で fail fast、(b) AAD（record_id ‖ version ‖ created_at）でレコード入替・ロールバック検出、(c) **ヘッダ全体に独立 AEAD タグ**を付与し `kdf_params` / `wrapped_VEK_*` の差替検出（Sub-D）。**ヘッダ AEAD タグの鍵経路は本書で凍結する: 鍵 = KEK_pw（パスワード由来 KEK そのもの、unwrap 直前段階で導出済の鍵を流用）**——別個 KDF を派生させると chicken-and-egg（ヘッダ復号に別鍵が必要 → その鍵をどこから持つかが循環）になる。**`kdf_params` の改竄は KDF 出力が変化するため、unwrap 失敗 → AEAD タグ不一致として間接検出される**（直接の改竄検出ではなく、復号経路の整合性で受動的に検出）。詳細実装契約は Sub-A / Sub-D で確定、(d) random nonce + 衝突上限 $2^{32}$ で同一 VEK 下の平文同値漏洩を確率的に排除、(e) `wrapped_VEK_*` を offline brute force するには Argon2id を超える必要あり（強パスワード前提）、**(f) Spoofing 対策**: IPC ピア UID 検証（Issue #26 実装済、`process-model.md` §4.2）、UDS `0700` + ディレクトリ親 `0700`、daemon シングルインスタンス保証（`flock` + unlink-before-bind、`process-model.md` §4.1）。**セッショントークンによる二重防御は後続 Issue 予定**（`threat-model.md` §7 Spoofing 行と整合、`IpcProtocolVersion::V2` の `Handshake { client_version, session_token }` 拡張で実装） |
| **残存リスク（受容）** | (1) **強パスワード前提**：弱パスワード（zxcvbn 強度 < 3）は本書では受容しない（Sub-D の `vault encrypt` 入口で Fail Fast 拒否）。`wrapped_VEK_*` のヘッダ部分は**読取できる**ため、強パスワードでも理論上 offline 試行は走らせられる（Argon2id `m=19456, t=2, p=1` 1 回当たり数百ms〜1 秒の作業証明が要件）。(2) **Spoofing 残存**: セッショントークン未実装期間は **daemon が起動済かつシングルインスタンス保証が機能している前提**でのみ Spoofing 防御が成立する。daemon クラッシュ → 再起動の race window では理論上 fake daemon の介入余地が残る。**この受容は後続 Issue（IPC V2 セッショントークン拡張）完了まで一時的なもの**として明示し、現時点では「daemon 起動失敗時 / IPC 接続失敗時はマスターパスワード入力経路を提示しない（Fail-Secure）」を Sub-E の IPC ハンドシェイクで担保 |
| **平文モード時の扱い** | **本書スコープ外**（`threat-model.md` §7.0 で受容済）。L1 攻撃で全レコード即時漏洩 |
| **テスト観点（Sub 受入の参照点）** | (a) 改竄注入 → `AeadTagMismatch` fail fast を確認（Sub-C / Sub-D）、(b) ヘッダ書換 → ヘッダ AEAD タグ検証で fail fast（Sub-D）、(c) random nonce 衝突確率の理論計算とベンチ（Sub-C）、**(d) CLI 攻撃面**: stdin パイプ拒否（C-38、TC-F-I12）/ core dump 抑制（C-41、TC-F-U10）/ env allowlist sanity check（C-40、TC-F-S05）/ `--output` フラグ排他指定（C-39、TC-F-U03）/ TTS dictation 学習回避（MSG-S18 受容前提）— Sub-F `test-design/sub-f-cli-subcommands/index.md` |
...
| **残存リスク** | 全敗（受容） |
| **平文モード時の扱い** | 該当なし — 受容根拠: L4 攻撃は OS 信頼境界そのものの崩壊であり、暗号化モード / 平文モードという vault 側の保護モード区別は**意味を持たない**。両モードとも全敗で受容（暗号化モードでも root 権限の ptrace で daemon メモリから VEK 抽出可能、平文モードはそもそも vault.db 直読みで全敗）。L1〜L3 における「平文モードでは保護不能」とは異なり、L4 では「**暗号化モードであっても保護不能**」が結論であることを Sub-A〜F の受入条件・テスト基準で取り違えないこと |
| **ユーザ向け約束** | SECURITY.md / `vault encrypt --help` / 初回 unlock 時の説明文 / **GUI 暗号化モード初回切替時のモーダル**（MSG-S16 / MSG-S17、`requirements.md`）で「**侵害された端末での使用は想定外**」「**root 権限を持つマルウェアからは保護できない**」を明示。過信防止が UX 責務（Fail Visible） |
...
| REQ-S08 | パスワード強度ゲート | `vault encrypt` 入口で zxcvbn 強度 ≥ 3 を Fail Fast チェック、強度不足時は `Feedback`（warning + suggestions）を CLI/GUI に提示（Fail Kindly） | Sub-A (#39) で `PasswordStrengthGate` trait と `WeakPasswordFeedback` 型を確定 + **Sub-B (#40) で `shikomi-infra::crypto::password::ZxcvbnGate` 具象実装**（Boy Scout Rule で旧 Sub-D 担当から再分配、Clean Arch 境界整合）+ Sub-D (#42) で `vault encrypt` 入口統合 + MSG-S08 i18n 翻訳 | 必須 |
...
| REQ-S14 | nonce overflow 検知 → rekey 強制 | 上限 $2^{32}$ 到達時 `NonceLimitExceeded` を返し、Sub-F の `vault rekey` で VEK 再生成 + 全レコード再暗号化 | Sub-A (#39) で `NonceCounter::increment` の `Result` 型契約と Boy Scout Rule 責務再定義 + Sub-C (#41) で AEAD 経路統合 + Sub-F (#44) で `vault rekey` フロー | 必須 |
...
| REQ-S16 | 保護モード可視化（CLI + GUI 両経路） | `shikomi list` 出力ヘッダで `[plaintext]` / `[encrypted]` を常時表示（Fail Visible）+ Tauri WebView の常駐バッジ（タイトルバー / トレイツールチップ / レコード一覧ヘッダ）で**色 + 文字の二重符号化**（色覚多様性対応）。CLI を読めないペルソナ A（田中）への必須対応、MSG-S17 で文言確定（`threat-model.md` §7.0 既定踏襲） | Sub-F (#44) + 後続 GUI feature | 必須 |
| REQ-S17 | Fail-Secure 型レベル強制 | `Verified<Plaintext>` newtype、`MasterPassword::new` の強度検証、`NonceCounter::increment` の `Result` 返却、`match` 暗号アーム第一パターン | Sub-A (#39) で 5 パターン全契約確定（C-1〜C-13） + Sub-B〜F で契約遵守（破ったら PR レビュー却下） | 必須 |
...
| **アクセシビリティ** | リカバリ 24 語の視覚障害ユーザ向け代替経路 | **WCAG 2.1 AA 準拠**: スクリーンリーダー対応（ARIA ロール / `aria-live="assertive"`）、ハイコントラスト印刷可能 PDF（`vault recovery-show --print`）、点字 `.brf` 出力（`--braille`）、OS TTS の録音禁止プレイヤー優先順位ガイド（`--audio`）の 4 経路を Sub-D / Sub-F が必ず実装。「初回 1 度きり表示」が視覚障害ユーザの完全敗北リスクに直結するため非機能要件に格上げ | 本書 §脅威モデル L4 受容前提（24 語紛失 = 永久損失）、REQ-S13 アクセシビリティ行、MSG-S18 |
| **ユーザ理解担保** | 暗号化モード切替時の限界説明 | CLI / GUI 両経路で「侵害端末で守れない」「root マルウェアで守れない」「BIP-39 漏洩で完全敗北」「画面共有中の表示回避」の 4 点を明示同意なしに次工程へ進ませない（MSG-S16） | 本書 §受入基準 #9、`threat-model.md` §7.0 既定 |
| **ユーザ理解担保** | 保護モード可視化（CLI + GUI 両経路） | `shikomi list` の `[plaintext]` / `[encrypted]` バナー（CLI、REQ-S16）+ Tauri WebView の常駐バッジ（GUI、MSG-S17）で**色（緑/灰）と文字（暗号化中/平文）の二重符号化**（色覚多様性対応）。CLI を読めないペルソナ A（田中）への必須対応 | 本書 §脅威モデル L1 / L3、`threat-model.md` §7.0 既定（Fail Visible） |
