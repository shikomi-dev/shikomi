...
| REQ-S08 | パスワード強度ゲート | `vault encrypt` 入口で zxcvbn 強度 ≥ 3 を Fail Fast チェック、強度不足時は `Feedback`（warning + suggestions）を CLI/GUI に提示（Fail Kindly） | Sub-A (#39) で `PasswordStrengthGate` trait と `WeakPasswordFeedback` 型を確定 + Sub-D (#42) で `shikomi-infra::crypto::ZxcvbnGate` 実装 | 必須 |
