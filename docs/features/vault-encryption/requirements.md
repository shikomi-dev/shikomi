...
| MSG-S07 | 成功 | **Sub-E 部分確定（応答スキーマ凍結、CLI 最終文言は Sub-F）**: 応答スキーマは `IpcResponse::Rekeyed { records_count, words, cache_relocked }`。文言指針: <br>**CLI 経路（Sub-F 最終確定）**: 「**vault を再暗号化しました**。再暗号化レコード数: N 件。新リカバリ用 24 語を以下に表示します（MSG-S06 警告連結）。`cache_relocked == false` 時は MSG-S20 を連結表示」<br>**GUI / 田中ペルソナ経路**: 「**鍵を新しくしました**。N 件のレコードを新しい鍵で再暗号化しました。新リカバリ用 24 語を表示します（MSG-S06 警告連結）。`cache_relocked == false` 時は MSG-S20 を連結表示」 | `vault rekey` 完了時（再暗号化レコード数 + 新 24 語 + cache_relocked を表示） |
...
| MSG-S14 | 確認 | **Sub-D 部分確定**（Sub-F が CLI 経路実装、確認ロジックも Sub-F 責務）: 「**警告: 暗号保護を解除しようとしています**」+ 3 点リスク明示 — (1)「**vault.db が平文モードに戻り、物理ディスク奪取で全レコードが平文化されます**」、(2)「BIP-39 24 語と Argon2id KDF による保護が**全件失われます**」、(3)「この操作は**取り消せません**（再暗号化には再度 `vault encrypt` 実行 + 新規 24 語生成が必要）」+ 二段確認「**`DECRYPT` と入力してください**」+ 「**マスターパスワードを再入力してください**」+ Sub-F が `subtle::ConstantTimeEq` で両方一致を判定 → 通過後 `DecryptConfirmation::confirm()` 呼出（**両方一致しないと進まない、`--force` でも省略不可**、`DecryptConfirmation` の型レベル強制 C-20、Sub-D Rev2 で確認ロジック責務を Sub-F に移譲）| `vault decrypt` 実行前 |
...
| MSG-S20 | 警告 | **Sub-E 確定（CLI / GUI 二経路分離、Sub-E 工程5 ペガサス致命指摘で追加）**: rekey / rotate_recovery の atomic write 完了直後に daemon 内 VEK 再キャッシュが失敗した場合の付帯警告（**Lie-Then-Surprise 防止の主役**、`basic-design/ux-and-msg.md` §cache_relocked: false の UX 設計判断 / 不変条件 C-30/C-31/C-32 と整合）。<br>**CLI 経路**: 「**注意**: 操作は完了しましたが、daemon の VEK 再キャッシュに失敗しました。次の操作前に `shikomi vault unlock` を再度実行してください」<br>**GUI / 田中ペルソナ経路**: 「**鍵情報の再読み込みに失敗しました**。次の操作前にもう一度パスワードを入力してください」（`VEK` / `daemon` を「鍵情報」に置換）。**内部詳細**（I/O エラー詳細 / KEK_pw 導出失敗詳細）は含めない。文言の不変条件: (a)「操作は完了した」を**先**に伝える（vault.db は正常、ロールバック不要）、(b)「再 unlock が必要」を**次に**伝える（次の一手の明示）、(c) `cache_relocked == false` 時も**新 24 語は表示する**（rekey / rotate_recovery の主目的、ユーザは紙にメモする責務）| `IpcResponse::Rekeyed { cache_relocked: false }` / `IpcResponse::RecoveryRotated { cache_relocked: false }` 受信時、Sub-F が MSG-S07 / S19 に**連結表示**する責務 |
