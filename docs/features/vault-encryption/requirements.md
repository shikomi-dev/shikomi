...
| 概要 | `shikomi vault {encrypt, decrypt, unlock, lock, change-password, recovery-show, rekey, rotate-recovery}` の **8 サブコマンド** CLI 実装、IPC V2 経由。**Sub-E 工程2 で `rotate-recovery` を追加確定**（mnemonic ローテーション専用、§F-E4） |
| 関連脅威 ID | L1（CLI → daemon は IPC、CLI 自身は VEK を持たない）／ L2（CLI プロセスは短命、メモリ滞留時間が daemon より小さい）／ L3（暗号化モード初回切替時の限界説明 MSG-S16 を CLI 経路で実装） |
| 入力 | (a) **`vault encrypt`**: マスターパスワード（プロンプト or stdin、TTY なら非エコー読取、`MasterPassword::new` で強度ゲート通過必須）+ 確認入力 + `--accept-limits` フラグ（MSG-S16 限界説明明示合意）、(b) **`vault decrypt`**: `DECRYPT` 文字列入力 + マスターパスワード再入力（**両方必須、`--force` でも省略不可**、C-20）、(c) **`vault unlock`**: マスターパスワード or `--recovery` 経路（24 語入力プロンプト、`bip39::Mnemonic::parse_in` で検証）、(d) **`vault lock`**: フィールドなし（`shikomi vault lock` 単体）、(e) **`vault change-password`**: 旧パスワード + 新パスワード + 新パスワード確認、(f) **`vault recovery-show`**: `--print` / `--braille` / `--audio` の代替経路フラグ（MSG-S18、WCAG 2.1 AA、`encrypt` 直後の 1 度のみ）、(g) **`vault rekey`**: マスターパスワード（VEK 入替契機: nonce overflow 自動 / ユーザ明示）、(h) **`vault rotate-recovery`**: マスターパスワード再認証 |
| 処理 | (1) clap 派生型 `Subcommand::Vault(VaultSubcommand)` で 8 サブコマンドを subcommand グループ化、(2) 各サブコマンドの usecase 層で `IpcClient::connect` → handshake V2 → `IpcRequest::*` 1:1 マップで送信、(3) **`vault decrypt` 二段確認**は CLI 層で `subtle::ConstantTimeEq` 比較 + paste 抑制（30ms 以内連続入力拒否）+ 大文字検証 → 通過後に `DecryptConfirmation::confirm()` 呼出 → IPC で `decrypt_vault` 呼出（C-20、Sub-D Rev2）、(4) **`vault recovery-show` C-19**: `IpcResponse::RecoveryRotated.words` または `Rekeyed.words` の `Vec<SerializableSecretBytes>` を 1 度だけ presenter に渡し、表示後即 `mem::replace` + Drop で zeroize、(5) **`cache_relocked: false` 経路**: `IpcResponse::Rekeyed/RecoveryRotated` の `cache_relocked` フィールドを presenter で分岐、`false` 時は MSG-S07/S19 + MSG-S20 連結表示 + 終了コード 0 + 次操作前に再 unlock 案内（C-32 能動的提示）、(6) **既存 `add/list/edit/remove` のロック時挙動**: `IpcResponse::Error(IpcErrorCode::VaultLocked)` 受信時に MSG-S09(c) 表示 + `shikomi vault unlock` 誘導 + 終了コード 1 で fail fast（REQ-S16 整合）、(7) **アクセシビリティ MSG-S18**: `vault recovery-show --print` で stdout に PDF を出力（`printpdf` クレートを `tech-stack.md` 凍結候補として Sub-F が確定）、`--braille` で `.brf` テキスト出力、`--audio` で OS TTS（macOS `say` / Windows `SAPI` / Linux `espeak`）への直接パイプ |
| 出力 | (a) stdout: 成功メッセージ（MSG-S01〜S07/S19 系列）、レコード一覧、recovery 24 語、(b) stderr: エラー（MSG-S08〜S15/S20）、警告（MSG-S06/S16/S18）、(c) **終了コード**: `0`（成功）/ `1`（一般エラー、内容含まず）/ `2`（パスワード違い、MSG-S09(a)）/ `3`（vault Locked、MSG-S09(c)）/ `4`（プロトコル非互換、MSG-S15）/ `5`（recovery-required、MSG-S09(a) リカバリ経路誘導）/ `64〜78`（`sysexits.h` `EX_USAGE` / `EX_DATAERR` / `EX_IOERR` 等の慣習継承）、(d) **MSG 文言出力先**: i18n 翻訳辞書 `shikomi-cli/locales/{ja-JP,en-US}/messages.toml` 経由（`detailed-design/cli-subcommands.md` §i18n 戦略責務分離） |
| エラー時 | Fail-Secure 必須: (a) IPC エラーをユーザに正確に伝達、内部詳細（KDF パラメータ / nonce カウンタ / スタックトレース / `failures` カウンタ値）は **CLI ユーザに見せず** `tracing::warn!` で stderr のみ（`--verbose` 時に有効化）、(b) **`vault decrypt` の二段確認スキップ不可**（`--force` フラグは提供しない、C-20 強制）、(c) **`recovery-show` 二度目以降は MSG-S09 系で fail fast**（C-19 所有権消費、daemon 側の `RecoveryDisclosure` 構築済フラグで判定、CLI には 1 度のみ表示）、(d) **`cache_relocked: false` 時に `Err` 終了コードを返さない**（C-31、operation は成功している、終了コード 0 + MSG-S20 連結警告のみ）、(e) **アクセシビリティモード検出**: `SHIKOMI_ACCESSIBILITY=1` 環境変数 / OS スクリーンリーダー検出 / 明示フラグのいずれかで `recovery-show` 出力経路を自動切替（MSG-S18） |
...
Sub-F (#44) で書き起こし。8 サブコマンド + 既存 4 サブコマンドのロック時挙動 + バナー表示の **CLI 表面契約**を以下に SSoT 化する。詳細フロー / clap 構造 / IPC 往復シーケンスは `detailed-design/cli-subcommands.md`、文言は `basic-design/ux-and-msg.md` を参照。

### vault サブコマンド表面契約

| サブコマンド | 引数 / フラグ | 入力経路 | 表示する MSG | 終了コード |
|------------|------------|---------|-----------|---------|
| `vault encrypt` | `[--accept-limits]` | プロンプト: マスターパスワード（非エコー）+ 確認 | MSG-S16（限界説明、`--accept-limits` なしなら明示合意求める）→ MSG-S08（弱い場合）→ MSG-S06（24 語警告）→ MSG-S01（完了）| 0 / 1 / 2 |
| `vault decrypt` | （フラグなし、`--force` 提供しない、C-20）| プロンプト: `DECRYPT` 文字列 + マスターパスワード再入力 | MSG-S14（二段確認）→ MSG-S02（完了）| 0 / 1 |
...
| `vault recovery-show` | `[--print \| --braille \| --audio]` | encrypt 完了直後の単発実行 | MSG-S06（警告連結）→ 24 語表示 → MSG-S18（アクセシビリティ案内）| 0 / 1（2 度目以降は失敗）|
| `vault rekey` | （フラグなし）| プロンプト: マスターパスワード | MSG-S11（nonce 上限到達契機）→ MSG-S07（完了 + 新 24 語）→ `cache_relocked: false` 時 MSG-S20 連結 | 0（cache_relocked: false でも 0、C-31）|
| `vault rotate-recovery` | （フラグなし）| プロンプト: マスターパスワード再認証 | MSG-S19（完了 + 新 24 語）→ `cache_relocked: false` 時 MSG-S20 連結 | 0（同上、C-31）|
...
| MSG-S18 | 警告 | **Sub-F 確定（CLI 出力実装 + 文言、Sub-D 指針凍結）**: **アクセシビリティ代替経路**（スクリーンリーダー利用ユーザ / 視覚障害ユーザがリカバリニーモニック 24 語を取り扱う際の代替手段案内）。OS 読み上げ拒否環境では以下の 3 経路から選択可能: (1)「**印刷可能なハイコントラスト PDF**（黒地白文字、最大 36pt、各語に番号付与）」: `vault recovery-show --print` で stdout に PDF バイナリ出力（`printpdf` 系を `tech-stack.md` 候補確定、Sub-F PR レビューで Boy Scout 選定）、(2)「**点字対応 `.brf`（Braille Ready Format）**」: `vault recovery-show --braille` で `.brf` テキスト出力、(3)「**録音禁止の音声プレイヤー優先順位ガイド**」: `vault recovery-show --audio` で OS 標準 TTS（macOS `say` / Windows `SAPI` / Linux `espeak`）を直接呼び録音可能アプリ経由の漏洩を回避。**自動切替経路**: `SHIKOMI_ACCESSIBILITY=1` 環境変数 / OS スクリーンリーダー検出 / 明示フラグのいずれかで `recovery-show` 出力経路を自動切替。**WCAG 2.1 AA 準拠**を非機能要件として固定（REQ-S13 末尾） | `vault recovery-show` 実行時にアクセシビリティモードが OS / shikomi 設定で検出された場合 |
