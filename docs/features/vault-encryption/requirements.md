...
| MSG-S03 | 成功 | **Sub-E 確定**: 「**vault をアンロックしました**。VEK は daemon メモリにキャッシュされ、アイドル 15 分 / スクリーンロック / サスペンドで自動的にロックされます」+ アクセシビリティ補足（`shikomi vault status` で残り秒数を確認可能、ただし数値非表示原則のため概算のみ表示）| `vault unlock` 成功時（IPC `Unlock` → `Unlocked` 応答受信） |
| MSG-S04 | 成功 | **Sub-E 確定**: 「**vault をロックしました**。VEK はメモリから消去されました（zeroize）。次回操作時は再度 `vault unlock` が必要です」| `vault lock` 完了時（明示 Lock IPC / アイドル 15min / OS スクリーンロック / サスペンド共通文言） |
| MSG-S05 | 成功 | **Sub-D + Sub-E 統合確定**: 「マスターパスワードを変更しました。**VEK は不変のため再 unlock は不要、レコード再暗号化も発生しません**。daemon キャッシュも維持されています（`vault status` で確認可能）」（O(1) change-password、REQ-S10 / `repository-and-migration.md` §F-D5 + Sub-E IPC V2 ハンドラ `change_password.rs` での daemon キャッシュ維持確定）| `vault change-password` 完了時 |
