...
| 入力 | (a) `&RecoveryMnemonic`（Sub-A 既存型、`expose_words_within_crate` 経由で 24 語の `&[String; 24]` 取得）、(b) BIP-39 passphrase は **Sub-B では未使用**（空文字列固定、`tech-stack.md` §2.4 凍結値 `salt='mnemonic'+''`）、(c) HKDF info 定数 `b"shikomi-kek-v1"`（const として shikomi-infra に固定） |
