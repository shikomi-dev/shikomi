...
| **Sub-E TC 総数** | **24 件**（ユニット 15 + 結合 7 + property 1 + E2E 1 = 24、Sub-A/B/C/D と同型レンジ）+ 静的検査 6 件（TC-E-S01..S06）|
...
| TC-E-U14 | C-28 / REQ-S12 | V1 クライアント handshake 後（`server_version: V1`）に V2 専用 variant `IpcRequest::Unlock` を MessagePack で送信 → `serde::Deserialize` が `unknown variant "unlock"` で失敗 → daemon 側で `IpcResponse::Error(IpcErrorCode::ProtocolDowngrade)` を返却（**`#[non_exhaustive]` の serde 経路保護**）| ユニット | 非破壊 |
| TC-E-U15 | EC-6 / REQ-S09 / S10 / S11 / S12 | i18n 翻訳辞書（`shikomi-cli/i18n/{ja,en}.toml` または同等）経由 MSG-S03（「vault をアンロックしました」+「アイドル 15 分 / スクリーンロック / サスペンドで自動的にロック」）/ MSG-S04（「vault をロックしました」+「VEK はメモリから消去」）/ MSG-S05（「VEK は不変のため再 unlock は不要」+「daemon キャッシュも維持」）/ MSG-S09 (a) パスワード違い（「リカバリニーモニックでの復号 (`vault unlock --recovery`) も可能」+ **`MigrationError::RecoveryRequired` 統合**）/ MSG-S15（「V1 クライアントは V2 専用機能を使用できません」+「`cargo install --force shikomi-cli`」更新案内）の文言を grep | ユニット | 文言 |
...
| TC-E-I07 | C-28 / REQ-S12 / EC-6 | (a) `MockClient { handshake_version: V1 }` で接続成功（V1 サブセット `ListRecords` / `AddRecord` / `EditRecord` / `RemoveRecord` のみ受理）、(b) V1 接続後に `IpcRequest::Unlock`（V2 専用）を MessagePack 送信 → `IpcResponse::Error(IpcErrorCode::ProtocolDowngrade)` 受信 + i18n 経由 MSG-S15「V1 クライアントは V2 専用機能を使用できません」「`cargo install --force shikomi-cli` で最新版に更新」を含む | 結合 | V1 非破壊 |
...
| TC-E-U14 | (1) handshake `IpcRequest::Handshake { client_version: V1 }` 送信 (2) `IpcResponse::Handshake { server_version: V1 }` 受信 (3) `IpcRequest::Unlock { .. }` を MessagePack バイト列にエンコード → V1 deserializer (`#[non_exhaustive]` の V1 enum 限定) で deserialize | (3) `Err(rmp_serde::decode::Error::SyntaxError("unknown variant \"unlock\""))` → daemon 側ハンドラが `IpcResponse::Error(IpcErrorCode::ProtocolDowngrade)` を返却（C-28、`#[non_exhaustive]` の serde 経路保護）|
...
これらは `tests/docs/sub-e-static-checks.sh` で実装する（Sub-D TC-D-S01..S07 と同パターン）。**Sub-D Rev3 / Rev4 で凍結した実装直読 + 機械検証**原則を Sub-E から継承し、`Locked → Unlocked → Locked` 型遷移の意味論ドリフトを Petelgeuse 5 度目を待たず構造封鎖する。
...
# Sub-E 静的検証 (cargo 不要、TC-E-S01..S06)
...
- 静的検証スクリプト stdout（`sub-e-static-checks.sh` 6 件: TC-E-S01..S06、Sub-D Rev4 同型構造防衛）
