...
<!-- 配置先: docs/features/vault-encryption/test-design/sub-0-threat-model.md -->
<!-- 親: docs/features/vault-encryption/test-design/index.md -->
<!-- 本書は Sub-0 (#38) 「脅威モデル文書化（L1〜L4 凍結）」のテスト設計のみを扱う。
     Sub-D (#42) Rev1 で `test-design.md` を `test-design/` ディレクトリに分割（500 行ルール準拠）。
     後続 Sub-E〜F のテスト設計は、各 Sub の工程2 で `test-design/sub-{e,f}-*.md` を新規作成する。 -->
