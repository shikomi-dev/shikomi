...
| REQ-S09 | `shikomi_daemon::cache::vek` | `crates/shikomi-daemon/src/cache/vek.rs` | `VekCache { state: Arc<tokio::sync::RwLock<VaultUnlockState>> }` + `VaultUnlockState` enum（`Locked` / `Unlocked { vek, last_used }`、`#[non_exhaustive]`、`Clone`/`Copy`/`Display`/`serde::Serialize` 未実装）+ `CacheError` 列挙型（`VaultLocked` / `AlreadyUnlocked`、`#[non_exhaustive]`） | **Sub-E 新規追加** |
| REQ-S09 | `shikomi_daemon::cache::lifecycle` | `crates/shikomi-daemon/src/cache/lifecycle.rs` | `IdleTimer`（60 秒ポーリングで `now - last_used >= 15min` 判定）+ `OsLockSignal` trait（`async fn next_lock_event() -> LockEvent`、`#[cfg]` 分割で macOS / Windows / Linux 具象実装、`MockLockSignal` テスト用）| **Sub-E 新規追加** |
| REQ-S11 | `shikomi_daemon::backoff::unlock` | `crates/shikomi-daemon/src/backoff/unlock.rs` | `UnlockBackoff { failures: u32, next_allowed_at: Option<Instant> }`（連続失敗 5 回で指数バックオフ 30s/60s/120s/.../1h クランプ、`record_failure` / `record_success` / `check`）| **Sub-E 新規追加** |
| REQ-S12 | `shikomi_daemon::ipc::v2_handler` | `crates/shikomi-daemon/src/ipc/v2_handler/{unlock,lock,change_password,rotate_recovery,rekey}.rs` | IPC V2 5 variant のハンドラ（各ハンドラ入口の `match VaultUnlockState` でワイルドカード `_` 禁止、`Locked => Err(IpcError::VaultLocked)` 強制で型レベル fail-secure C-22）+ `MigrationError → IpcError` マッピング（9 → 4 集約、`RecoveryRequired → MSG-S09(a)` 統合、Sub-D Rev5 ペガサス指摘契約の実装）| **Sub-E 新規追加** |
| REQ-S12 | `shikomi_core::ipc` | `crates/shikomi-core/src/ipc/{protocol,request,response,error}.rs` | `IpcProtocolVersion::V2` 非破壊昇格 + `IpcRequest::{Unlock, Lock, ChangePassword, RotateRecovery, Rekey}` 5 新 variant + `IpcResponse::{Unlocked, Locked, PasswordChanged, RecoveryRotated, Rekeyed}` 5 新 variant + `IpcError::{VaultLocked, BackoffActive, RecoveryRequired, ProtocolDowngrade}` 4 新 variant。**SSoT は `daemon-ipc/detailed-design/protocol-types.md`**、本書は消費側として参照 | **Sub-E で改訂**（横断的変更、`daemon-ipc` feature への影響） |
