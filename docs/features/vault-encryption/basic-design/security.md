...
| **L2**: メモリスナップショット | コアダンプ / ハイバネーションファイル / スワップから VEK / KEK / MasterPassword / 平文抽出 | `Vek` / `Kek` / `MasterPassword` / `RecoveryMnemonic` / `Plaintext` | 全て `secrecy::SecretBox` ベース、`Drop` 連鎖で**派生集約も連動消去**。`Clone` を**意図的に未実装**（誤コピーで滞留時間延長を構造禁止）。`Debug` は `[REDACTED ...]` 固定、`Display` 未実装、`serde::Serialize` 未実装（コンパイル時に誤シリアライズを排除）。**Sub-E 追加**: `VekCache` (`tokio::sync::RwLock<VaultUnlockState>`) で daemon 内 VEK 寿命を管理、(a) アイドル 15 分タイムアウト（契約 C-24）、(b) OS スクリーンロック / サスペンド受信で即 lock（契約 C-25、`OsLockSignal` trait 経由）、(c) `Locked → Unlocked` 遷移時に `mem::replace` で旧 state 取り出し → `Vek::Drop` で 32B zeroize（契約 C-23）、(d) `Locked` 状態で read/write IPC は `match VaultUnlockState` でワイルドカード `_` 禁止 + `Locked => Err(IpcError::VaultLocked)` で型レベル拒否（契約 C-22、Sub-C `decrypt_record` C-14 同型 fail-secure） |
...
| A07 | Auth Failures | **Sub-E で完結**。`MasterPassword` の強度検証契約（Sub-A trait + Sub-B `ZxcvbnGate` 具象実装）、`UnlockBackoff` で連続失敗 5 回時の指数バックオフ（30s/60s/120s/.../1h クランプ、契約 C-26）、`MigrationError::RecoveryRequired` → `IpcError::RecoveryRequired` 透過 → MSG-S09 (a) リカバリ経路案内（契約 C-27）、`tokio::time::sleep` は `unlock.rs` ハンドラ内のみで blocking、daemon 全体の `Future` を停止しない（ホットキー応答継続） |
