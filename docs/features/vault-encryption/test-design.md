...
| テストID | 受入基準 / REQ | 検証内容 | レベル | 種別 |
|---|---|---|---|---|
| TC-B-U01 | BC-1 / REQ-S03 / L3 | `Argon2idAdapter::derive_kek_pw` が RFC 9106 Test Vector A.2 と bit-exact 一致（出力 32B） | ユニット | KAT |
...
| TC-B-U05 | BC-4 / REQ-S04 / L3 | `Bip39Pbkdf2Hkdf::derive_kek_recovery` が trezor 公式 vectors（24 語 + 期待 seed + 期待 KEK_recovery）と bit-exact 一致（最低 5 ベクトル） | ユニット | KAT |
| TC-B-U06 | REQ-S04 | BIP-39 wordlist + checksum 検証: `Mnemonic::parse_in(English, ...)` が不正 24 語で `InvalidMnemonic` Err | ユニット | 異常系 |
| TC-B-U07 | BC-6 / REQ-S04 | HKDF-SHA256 RFC 5869 Appendix A.1〜A.3 KAT bit-exact 一致（3 ベクトル） | ユニット | KAT |
...
|---|---|---|
| TC-B-U01 | RFC 9106 Test Vector A.2 の `(password: 32B, salt: 16B, m=19456, t=2, p=1)` | `derive_kek_pw` 戻り値 = ベクトル期待 32B（bit-exact） |
| TC-B-U05 | trezor `vectors.json` 24 語 + 期待 seed (64B) + 期待 KEK_recovery (32B、HKDF info=`b"shikomi-kek-v1"`) | `derive_kek_recovery` 戻り値 = 期待 32B（最低 5 ベクトル）|
| TC-B-U07 | RFC 5869 Appendix A.1: IKM/salt/info/L → expected OKM | HKDF-SHA256 戻り値 = 期待 OKM |
