...
| 本テスト設計のスコープ | **Sub-0 (#38) + Sub-A (#39) + Sub-B (#40) + Sub-C (#41) + Sub-D (#42)** — Sub-0: 脅威モデル文書（§1〜§9）、Sub-A: 暗号ドメイン型（§10）、Sub-B: KDF + Rng + ZxcvbnGate（§11）、Sub-C: AEAD アダプタ（§12）、**Sub-D: 暗号化 Vault リポジトリ + 平文⇄暗号化マイグレーション + REQ-P11 改訂（§13）**。Sub-E/F は本ファイルを順次 READ → EDIT で拡張 |
| 対象 Issue | [#38](https://github.com/shikomi-dev/shikomi/issues/38) / [#39](https://github.com/shikomi-dev/shikomi/issues/39) / [#40](https://github.com/shikomi-dev/shikomi/issues/40) / [#41](https://github.com/shikomi-dev/shikomi/issues/41) / [#42](https://github.com/shikomi-dev/shikomi/issues/42) |
| 対象ブランチ | `feature/issue-38..41` 全マージ済 / `feature/issue-42-encrypted-vault`（Sub-D、本工程）|
...
| **本テスト設計の TC 総数** | **Sub-0: 26 件** + **Sub-A: 22 件** + **Sub-B: 25 件** + **Sub-C: 26 件** + **Sub-D: 26 件**（TC-D-U01〜U18 / I01〜I05 / P01〜P02 / E01）= **合計 125 件**。各 Sub のローカル META チェックは独立に管理（ID prefix で分離） |
...
| TC-D-P02 | C-17 / property | proptest で encrypt 後、(a) ヘッダ任意 byte flip、(b) per-record 任意 byte flip、(c) ヘッダ AEAD タグ任意 byte flip、(d) per-record AEAD タグ任意 byte flip の 4 経路全てで **`MigrationError::Crypto(AeadTagMismatch)` を確実に検出**（1000 ケース）| property | 改竄検出 |
...
| TC-D-P02 | encrypt 完了後の vault.db バイト列に対し、(a) ヘッダ任意 byte flip、(b) per-record ciphertext 任意 byte flip、(c) ヘッダ AEAD タグ任意 byte flip、(d) per-record AEAD タグ任意 byte flip の 4 経路 | 全経路で `decrypt_vault` または `unlock_with_password` が **必ず `Err(MigrationError::Crypto(AeadTagMismatch))` を返す**（1000 ケース、改竄経路を 4 軸全網羅）|
