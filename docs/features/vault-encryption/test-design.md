...
| TC-A-U18 | REQ-S17 | `CryptoOutcome<T>` enum バリアント `Verified(Verified<T>)` / `Tampered(CryptoError)` / `KeyMissing(...)` 等が網羅、`match` 強制で fall-through なし | ユニット | enum 網羅 |
...
| TC-A-U18 | `CryptoOutcome<T>` `match` 網羅 | enum 網羅 | 全バリアントを列挙 | `match` で fall-through が `cargo check` で警告（`#[non_exhaustive]` 適用、外部 crate からの追加で破壊的変更にならない） |
