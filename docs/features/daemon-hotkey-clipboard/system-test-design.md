...
本 feature のシステムテストは「ホットキー押下 → クリップボード書き込み」の **エンドツーエンド経路**を検証する。ホットキーイベントは CI 環境でグローバルホットキー受信不可のため、**IPC 直接呼び出しによるクリップボード書き込み経路**を E2E テスト代替とする。実 OS ホットキーの確認は手動受入テスト（AC-HK-01〜05）で補う。
...
| 前提 | daemon が起動済み |
| 手順 | ① `shikomi add --hotkey "ctrl+alt+1" --label "e2e" --value "hello-e2e"` ② daemon へ `IpcRequest::TriggerHotkey { combo: "ctrl+alt+1" }` を直接送信（テスト専用 IPC） ③ OS クリップボードの内容を `arboard` で読み取る |
| 期待結果 | クリップボードに `"hello-e2e"` が書き込まれている |
| CI 実行 | `test-daemon.yml` に追加（arboard のヘッドレス CI 動作確認後） |
...
| 対応要件 | R1-HK-05 |
| 前提 | TC-HK-E02 の環境 |
| 手順 | ① `shikomi add --hotkey "ctrl+alt+2" --label "pw" --secret` → stdin でパスワード入力 ② `IpcRequest::TriggerHotkey { combo: "ctrl+alt+2" }` 送信 ③ 30 秒後にクリップボードを読み取る |
| 期待結果 | クリップボードが空文字またはデータなし状態 |
| CI 実行 | タイムアウト設定（60s）で `test-daemon.yml` に追加 |
...
### TC-HK-E05: ロック中 vault のホットキー押下はサイレントスキップ

| 項目 | 内容 |
|------|------|
| テスト ID | TC-HK-E05 |
| 対応要件 | R1-HK-07 |
| 手順 | ① 暗号化 vault で daemon 起動（ロック状態） ② `IpcRequest::TriggerHotkey` を送信 ③ クリップボードを確認 |
| 期待結果 | クリップボードに変化なし。エラーログ出力なし（サイレント） |
| CI 実行 | `test-daemon.yml`（Sub-E fixture 流用） |
